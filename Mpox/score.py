...
def similarity_score(embedding1, embedding2):
    ...
    """
    # 点积
    dot_product = sum(e1 * e2 for e1, e2 in zip(embedding1, embedding2)) # zip将列表转成一一对应的元组
    # 向量的模长
...
def get_score(options):
    val_path = options['val_path'] # 验证集（包含题目和参考答案）
    ans_path = options['val_out_path'] # 验证集题目的生成答案
    f_len = count_lines_in_jsonl(val_path) # 读取行数，并将每一行转成json对象
    val_json = []
    ans_json = []
    # 先读取两个文件的数据
